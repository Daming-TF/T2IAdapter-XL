...
# # V2 exp - [multi image prompt embedding]
# from .attention_processor import IPAttnProcessor2_1 as IPAttnProcessor

# V1_1 exp - [add img feature in diff position]
from .attention_processor import IPAttnProcessor2_2 as IPAttnProcessor
...
# ToDo:
#  exp - [img embedding add to diff blocks]
#  ps: alse suport controlnet
class IPAdapterV1(IPAdapterPlus):
    def generate(
        ...
        if isinstance(pil_image, Image.Image):
            ...
            print('pil image is only supported input PIL Image')
            exit(0)
        ...
        with torch.inference_mode():
            ...
            # add new ———— suport add img feature in diff blocks
            prompt_embeds_list = [
                prompt_embeds_,
                prompt_embeds,
            ]
...
# ToDo:exp - [multi image inference]
class IPAdapterV2(IPAdapterPlus):
    def generate(
            self,
            pil_images,
            prompt=None,
            ...
            guidance_scale=7.5,
            num_inference_steps=30,
            feature_mode='simple',  # 'simple', 'avg_embeds', 'token_concat', 'avg_feature'
            **kwargs,
    ):
        self.set_scale(scale)
        num_prompts = 1
        ...
        with torch.inference_mode():
            ...
            for image_prompt_embeds, uncond_image_prompt_embeds in zip(image_prompt_embeds_list,
        ...
        images = self.pipe(
            ...
            negative_prompt_embeds=negative_prompt_embeds_list,
            guidance_scale=guidance_scale,
            num_inference_steps=num_inference_steps,
            generator=generator,
            **kwargs,
