...
if __name__ == "__main__":
    ...
    guidance_start_list = list(np.arange(0, 0.6, 0.1))
    print(scales)
    ...
    for image_path in image_paths:
        ...
        for prompt in prompts:
            ...
            for scale in scales:
                ...
                v_concat = None
                for guidance_start in guidance_start_list:
                    controlnet_params['guidance_start'] = guidance_start
                    ...
                    print(f'***********************\n'
                          ...
                          f" ** guidance_start:\t\t{controlnet_params['guidance_start']}\n"
                          f" ** guidance_end:\t\t{controlnet_params['guidance_end']}\n"
                          )
                    img = np.array(test_server(args, status_dict, params))
                    v_concat = cv2.vconcat([v_concat, img]) if v_concat is not None else img

                result = cv2.hconcat([result, v_concat]) if result is not None else v_concat

            save_path = os.path.join(save_dir_0, f"{prompt.replace(' ', '_').replace(',', '-')}.jpg")
