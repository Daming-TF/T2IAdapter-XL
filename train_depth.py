...
# from tool.inference_sample import inference_sample
...
from tool.sample_util import inference
from tool.model_util import save_sdxl_adapter_checkpoint


def trans_tensor2img(tensor_image):
    tensor_image *= 255
    tensor_image = tensor_image.byte()  # to uint8
    if tensor_image.dim() == 2:     # gray-depth
        tensor_image = tensor_image.unsqueeze(2)
    # elif tensor_image.dim() == 3:
    #     tensor_image = tensor_image.permute(1, 2, 0)        # [C, H, W] to [H, W, C]
    # else:
    #     ValueError("input tensor dim must be 2 or 3")
    #     exit(1)
...
def inference_sample(
        ...
        adapter, config, args, test_dataloader, epoch, step):
    # negative prompt
    neg_prompt = ''
    ...
    # sampler = diffusion_inference(args.pretrained_model_name_or_path)
    ...
    for index, batch in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            adapter_features \
                = adapter(copy.deepcopy(cond).to(dtype=weight_dtype).cuda(device))
            result = inference(
                tokenizers, scheduler, model, vae, text_encoders, device, weight_dtype,
                prompt=prompt,
                prompt_n=neg_prompt,
                steps=50,
                adapter_features=[sample.to(dtype=weight_dtype) for sample in copy.deepcopy(adapter_features)],
                guidance_scale=7.5,
        ...
        root_results = os.path.join(args.output_dir, cond_name)
        ...
        # time_suffixes = formatted_date + '-' + formatted_time

        im_cond = np.array(trans_tensor2img(cond.squeeze(0)))
        ori_img = np.array(trans_tensor2img(img.squeeze(0)))

        cv2.imwrite(
            os.path.join(root_results, f'e{epoch}_i{step}_{index}_ori' + str(prompt) + '.png'),
            cv2.cvtColor(ori_img, cv2.COLOR_RGB2BGR)
        )
        cv2.imwrite(
            os.path.join(root_results, f'e{epoch}_i{step}_{index}_res' + str(prompt) + '.png'),
            cv2.cvtColor(result, cv2.COLOR_RGB2BGR)
        )
        cv2.imwrite(
            os.path.join(root_results, f'e{epoch}_i{step}_{index}_cond' + str(prompt) + '.png'),
            cv2.cvtColor(im_cond, cv2.COLOR_RGB2BGR)
        )

        # wandb
        merged_image = cv2.hconcat([ori_img, result, np.stack([im_cond] * 3, axis=-1)])
        merged_image = cv2.resize(merged_image, (int(merged_image.shape[1] * 0.5), int(merged_image.shape[0] * 0.5)))
        # merged_image = cv2.cvtColor(merged_image, cv2.COLOR_RGB2BGR)
        samples[f'index{index}'] = [wandb.Image(merged_image, caption=prompt)]
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/train/Adapter-XL-depth.yaml",
...
def main(args):
    ...
    args.train_batch_size = config.data.params.train.batch_size
    ...
    start_global_step = 0
    start_epoch = 0
    if config.model.params.adapter_config.pretrained is not None:
        start_epoch, start_global_step, state_dict = load_sdxl_adapter_chaeckpoit(config.model.params.adapter_config.pretrained)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(adapter):
                ...
                batch["jpg"] = batch["jpg"]*2.-1.       # [0, 1] transition to [-1, 1]?
                # get depth
                ...
                # original——cubic      Pattern error:CUDA errordevice-side assert triggered
                timesteps = torch.rand((bsz, ), device=latents.device)
                timesteps = (1 - timesteps**3) * noise_scheduler.config.num_train_timesteps
                timesteps = timesteps.long()
                timesteps = torch.clamp(timesteps, min=0, max=max_timestep)     # my add
            ...
            if accelerator.sync_gradients:
                ...
                # if accelerator.is_main_process and global_step % config['logger']['checkpointing_steps'] == 0:
                ...
                if accelerator.is_main_process and global_step % config['logger']['checkpointing_steps'] == 0:
                    ckpt_save_dir = os.path.join(args.output_dir, 'chenckpoint')
                    os.makedirs(ckpt_save_dir, exist_ok=True)
                    save_path = os.path.join(ckpt_save_dir, f'e{epoch}-i{global_step}.ckpt')
                    save_sdxl_adapter_checkpoint(
                        save_path,
            ...
            # # record epoch and step info
            # if global_step % config['logger']['print_freq'] == 0 and accelerator.is_main_process:
            #     iter_state = {'epoch': epoch, 'iter': global_step}
            #     # combined_dict = {**logs, **iter_state}
            #     logger.info(iter_state)

            # sample
            if global_step % config['logger']['sample_freq'] == 0:
                accelerator.wait_for_everyone()
                if accelerator.is_main_process:
                    with torch.no_grad():
                        inference_sample(
                            tokenizers, noise_scheduler, unet, vae, text_encoders, accelerator.device, weight_dtype,
                            adapter, config, args, test_dataloader, epoch, global_step)
