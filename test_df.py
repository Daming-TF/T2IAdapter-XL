...
"""
    TODO： How do ckpt tests saved for different iteration steps work in the test set
"""
...
if __name__ == '__main__':
    ...
    depth_paths = [os.path.join(depth_dir, name) for name in os.listdir(depth_dir) if '.depth.png' in name]
    for depth_path in depth_paths:
        # depth_path = r'/mnt/nfs/file_server/public/mingjiahui/data/inference_test/000000000785.depth.png'
        depth = Image.fromarray(cv2.imread(depth_path)).convert('L')
        cond = image_transforms(depth).to('cuda').unsqueeze(0)
        with open(depth_path.replace('.depth.png', '.txt')) as f:
        ...
        # save results
        root_results = os.path.join('/home/mingjiahui/data', cond_name)
        ...
        save_path = os.path.join(root_results, f"{os.path.basename(depth_path).split('.')[0]}_seed{global_opt.seed}_image.png")
